...
func (req ListRequest) Validate() error {
	return validation.ValidateStruct(&req,
		...
		validation.Field(&req.PluginIDs, validation.Each(validationutil.UUIDRule(false)...)),
		validation.Field(&req.OrderField, validation.In(OrderFieldCreatedAt, OrderFieldExecutionOrder).Error("must be one of 'created_at' or 'execution_order'")),
		validation.Field(&req.OrderDirection, validation.In("asc", "desc").Error("must be either 'asc' or 'desc'")),
