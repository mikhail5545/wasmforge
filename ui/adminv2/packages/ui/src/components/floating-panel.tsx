      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              "fixed z-50 overflow-hidden shadow-lg outline-none",
              className
            )}
            style={{
              borderRadius: 12,
...
function FloatingPanelTitle({ children }: FloatingPanelTitleProps) {
  ...
  return (
    <motion.div
      layoutId={`floating-panel-label-container-${uniqueId}`}
      className="px-4 py-2 bg-foreground"
    >
      <motion.div
        layoutId={`floating-panel-label-${uniqueId}`}
...
export function FloatingPanelCloseButton({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.9 }}
    >
      <ArrowLeftIcon size={16} className="text-background" />
    </motion.button>
  )
}
...
export function FloatingPanelSubmitButton({
  ...
  return (
    <motion.button
      className={cn(
        "relative ml-1 flex h-8 shrink-0 scale-100 select-none appearance-none items-center justify-center rounded-lg border border-foreground/10 bg-foreground px-2 text-sm text-background transition-colors  focus-visible:ring-2 active:scale-[0.98]  dark:text-zinc-50 hover:bg-background/5",
        className
      )}
      type="submit"
...
export function FloatingPanelButton({
  ...
  return (
    <motion.button
      className={cn(
        "flex w-full items-center gap-2 rounded-md px-4 py-2 text-left text-sm bg-foreground",
        className
      )}
      onClick={onClick}
